                ...
                <div class="card-body">
                  <div class="row align-items-center">
                    ...
                    <div class="col col-stats ms-3 ms-sm-0">
                      <div class="numbers">
                        <p class="card-category">My Patients</p>
              ...
              <div class="col-md-12">
                <div class="card">
                  ...
                  <div class="card-body" style="height: 300px; width:100%">
                    <canvas id="appointmentStatusPieChart" width="400" height="300"></canvas>
                  </div>
                </div>
              </div>
            </div>
  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", async () => {
      try {
        ...
        const endpoints = {
          overview: 'https://clinicmsys-production.up.railway.app/api/dashboard/doctoroverview',
          appointmentStats: 'https://clinicmsys-production.up.railway.app/api/dashboard/appointmentsStatus',
          appointmentTime: 'https://clinicmsys-production.up.railway.app/api/dashboard/appointmentsOverMonths'
        };
        ...
        const [overviewData, appointmentData, appointmentTime] = await Promise.all(responses.map(res => res.json()));

        // Extracting data
        const dashboardData = overviewData.data;
        const appointmentStats = appointmentData.data;
        const appointmentOverTime = appointmentTime.data;
        // Update Dashboard Overview Data
        document.getElementById('numberOfPatients').innerText = dashboardData.totalPatients;
        document.getElementById('numberOfTodayAppointments').innerText = dashboardData.todayAppointments;
        document.getElementById('numberOfAppointments').innerText = dashboardData.totalAppointments;
        // Update Charts and Tables
        updateChart(dashboardData);
        updateAppointmentPieChart(appointmentStats);
        renderAppointmentsChart(appointmentOverTime);
      } catch (error) {
    ...
    function renderAppointmentsChart(data) {
      // Extract labels (months) and data (appointment count)
      ...
      // Destroy existing chart if already exists (to prevent duplication)
      if (window.myAppointmentsChart) {
        window.myAppointmentsChart.destroy();
      }

      // Create the chart
      window.myAppointmentsChart = new Chart(ctx, {
        type: 'bar', // Use 'line' if you prefer a line chart
        data: {
          labels: labels, // X-axis labels (months)
          datasets: [{
            label: 'Appointments Per Month',
            data: counts, // Y-axis data (appointment count)
