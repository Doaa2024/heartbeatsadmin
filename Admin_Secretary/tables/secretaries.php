...
<body>
  <div class="wrapper">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body fs-5">
            Are you sure you want to remove this secretary?
          </div>
    ...
    <div class="modal fade" id="editRowModal" tabindex="-1" aria-labelledby="editRowModal" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="addBranchModalLabel">Edit Secretary</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="color: red;"></button>
          </div>
          <div class="modal-body">
            <form id="editForm">
              <div class="form-group">
                <label for="coursetitle">Secretary Name</label>
                <input type="text" class="form-control shadow-blue" id="nameE" name="name" required>
              </div>
              <div class="form-group">
                <label for="coursetitle">Secretary Email</label>
                <input type="text" class="form-control shadow-blue" id="emailE" name="email" required>
              </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-primary" form="editForm">Edit Secretary</button>
          </div>
          </form>
        </div>
      </div>
    ...
    <div class="modal fade" id="addRowModal" tabindex="-1" aria-labelledby="addRowModal" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="addBranchModalLabel">Add New Secretary</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="color: red;"></button>
          </div>
          <div class="modal-body">
            <form id="addForm">
              ...
              <div class="form-group">
                <label for="coursetitle">Secretary Password</label>
                <input type="text" class="form-control shadow-blue" id="password" name="password" required>
              </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-primary" form="addForm">Add Secretary</button>
          </div>
          </form>
        </div>
      </div>
      ...
      <div class="container">
        <div class="page-inner">
          <div class="page-header">
            <h3 class="fw-bold mb-3">Secretaries</h3>
              ...
              <div class="card">
                <div class="card-header">
                  <div class="d-flex align-items-center">
                    <h4 class="card-title">Secretaries</h4>
                    <button
  ...
  <script>
    ...
    document.addEventListener("DOMContentLoaded", () => {
      fetchData('https://clinicmsys-production.up.railway.app/api/secretary');
    });

    function fetchData(url = 'https://clinicmsys-production.up.railway.app/api/secretary') {
      ...
      fetch(url, {
        ...
        .then((response) => {
          if (!response.ok) {
            // If the response status is unauthorized (401 or 403), handle it
            if (response.status === 401 || response.status === 403) {
              // Show the Swal alert for Unauthorized
              Swal.fire({
                icon: 'error',
                title: 'Unauthorized',
                text: 'You are not authorized to perform this action.',
                showConfirmButton: false, // Don't show the button to dismiss
                allowOutsideClick: false, // Prevent closing the alert outside
                allowEscapeKey: false, // Prevent closing the alert using the ESC key
              });
        ...
        .catch((error) => {
          // If the error was not due to unauthorized, handle it here
                ...
                <td>
                    <div class="form-button-action">
                        <button type="button" data-bs-toggle="modal" data-bs-target="#editRowModal"
                            class="btn btn-link btn-primary btn-lg  editbutton" data-item="${item.id}"  data-name="${item.name}"  data-email="${item.email}" >
    ...
    function handleDelete(id) {
      ...
      fetch(`https://clinicmsys-production.up.railway.app/api/secretary/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
        ...
        .then(response => {
          if (response.ok) {
            ...
            Swal.fire({
              icon: 'success',
              title: "Secretary deleted Successfully",
    ...
    document.getElementById('addForm').addEventListener('submit', async function(event) {
      ...
      try {
        ...
        const response = await fetch('https://clinicmsys-production.up.railway.app/api/secretary', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
        ...
        if (response.ok) {
          Swal.fire({
            icon: 'success',
            title: "Secretary added successfully!",
            showConfirmButton: true,
            customClass: {
              confirmButton: 'button btn btn-primary app_style'
            }
    ...
    $(document).ready(function() {
      ...
      $('#editForm').on('submit', function(e) {
        ...
        $.ajax({
          url: `https://clinicmsys-production.up.railway.app/api/secretary/${IDToUpdate}`,
          ...
          success: function(response) {
            ...
            Swal.fire({
              icon: 'success',
              title: "Secretary updated successfully",
              showConfirmButton: true,
              customClass: {
                confirmButton: 'button btn btn-primary app_style'
              }
